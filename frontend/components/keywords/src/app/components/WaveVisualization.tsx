...
export function WaveVisualization() {
  ...
  useEffect(() => {
    ...
    // Use a timeout to ensure SVG is rendered
  ...
  // Update width when focused mode or comment panel changes
  useEffect(() => {
    const updateWidth = () => {
      if (svgRef.current) {
    ...
    // Multiple updates to catch the transition
  ...
  const handleNodeClick = (nodeId: string) => {
    if (focusedNode === nodeId) {
      // Clicking the same focused node clears focus
      setFocusedNode(null);
      setSelectedNodes(new Set());
      setDetailView('connection');
      setDetailNodeId(null);
    } else {
      // Enter focused mode for this node
  ...
  const handleClear = () => {
    // If we have a saved canvas state, restore it
    if (savedCanvasState) {
  ...
  const handleConnectionClick = (connection: { title: string; description?: string }, doctorInfo?: { name: string; avatar: string; specialty: string; doctorId: number; postId: number }) => {
    ...
    // Parse the connection pattern and load mock nodes
    // In a real app, this would parse the actual connection data
    // For now, we'll simulate loading a connection by selecting specific nodes
    
    // Example: "Imaging Studies → Lab Monitoring → Radiation"
    // We'll select relevant nodes from the canvas
    ...
    // Store doctor connection info if provided
    if (doctorInfo) {
  ...
  const handleNodeHover = (nodeId: string | null) => {
    if (focusedNode) return; // Don't allow hover in focused mode
  ...
  const handleVoicePopupClick = () => {
    // Mock transcription - add sample medical comment
  ...
  const getNodePosition = (columnIndex: number, nodeIndex: number, totalNodes: number) => {
    // Calculate responsive column positioning to span full width
    const horizontalPadding = 40;
    const availableWidth = svgWidth - (horizontalPadding * 2);
    const totalColumns = medicalData.length;
    
    // Calculate spacing to distribute columns across full width
  ...
  // Get horizontal focused position
  const getHorizontalPosition = (index: number, total: number) => {
    // Use the state value for SVG width which updates when panel opens/closes
    const actualSvgWidth = svgWidth;
    
    // Node dimensions
    const nodeWidth = columnWidth;
    const minSpacing = 20; // Minimum gap between nodes
    
    // Fixed maximum of 4 nodes per row
    const maxNodesPerRow = 4;
    const nodesPerRow = Math.min(total, maxNodesPerRow);
    ...
    // Calculate spacing for this row to center it
    ...
    // Center the row within the actual SVG width
    ...
    const startY = 350 - totalHeight / 2; // Center vertically
  ...
  const getConnections = (nodeId: string): string[] => {
    ...
    return [...new Set(allConnections)]; // Remove duplicates
  };
  ...
  const isConnectionVisible = (fromId: string, toId: string): boolean => {
    if (focusedNode) {
      // In focused mode, only show connections to/from focused node
  ...
  const generateCurvePath = (
    ...
    if (focusedNode) {
      // Horizontal layout - use different curve
      const midY = (y1 + y2) / 2;
  ...
  // Get visible nodes for focused mode
                ...
                {focusedNode && commentPanelOpen && (
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200">
                    ...
                    <form onSubmit={handleAddComment} className="flex items-end gap-2 relative">
                      ...
                      {/* Voice Recording Button with Popup */}
                      <div className="relative">
                        <button
                          type="button"
                ...
                {!focusedNode ? (
                    ...
                    {medicalData.map((column, colIndex) =>
                      column.nodes.map((node) => {
                        ...
                        return node.connections.map((targetId) => {
                          ...
                          // Calculate global animation delay
                          ...
                          const allNodesAppearTime = (totalNodes - 1) * nodeStaggerDelay + nodeAnimationDuration + 0.15; // Add buffer
                    ...
                    {medicalData.map((column, columnIndex) => {
                      ...
                      <g key={columnIndex}>
                        ...
                        {column.nodes.map((node, nodeIndex) => {
                          ...
                          // Calculate global node index for staggered animation
                  ...
                  <g key="focused-view">
                    {(() => {
                      ...
                      // Sort nodes by column index to show in order
                      const sortedNodes = [...visibleNodes].sort((a, b) => a.columnIndex - b.columnIndex);
                      
                      // Find the focused node's new index in sorted array
                      const focusedIndex = sortedNodes.findIndex(node => node.id === focusedNode);
                      
                      // Calculate delay for connections: wait for all nodes to appear first
                      const nodeAnimationDuration = 0.5;
                      const nodeStaggerDelay = 0.1;
                      const panelAnimationDelay = commentPanelOpen ? 0.5 : 0; // Extra delay when panel is animating
