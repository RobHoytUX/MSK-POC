...
export interface TrialMatchingCriteriaFlowProps {
  patientPool: Patient[];
  variant: "panel" | "page";
  onViewPatientQualification: (patient: Patient) => void;
  onClose?: () => void;
  /** When set, results step shows "Continue to home" with these qualified patient IDs. */
  onPageComplete?: (qualifiedPatientIds: string[]) => void;
}

export function TrialMatchingCriteriaFlow({
  patientPool,
  variant,
  onViewPatientQualification,
  onClose,
  onPageComplete,
}: TrialMatchingCriteriaFlowProps) {
  ...
  }, [patientPool, criteria, biomarkers]);
  ...
  const qualifiedIds = useMemo(
    () =>
      results
        .filter((r) => r.score >= 70 && !r.flags.some((f) => f.toLowerCase().includes("exclusion")))
        .map((r) => r.patient.id),
    [results]
  );
  ...
  const rootClass =
    variant === "panel"
      ...
      : "h-full min-h-0 flex flex-col bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden";
  ...
  return (
    <div className={rootClass}>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {STAGE_OPTIONS.map((st) => (
                    <CheckPill
                      key={st}
                      label={st}
                      active={criteria.stages.includes(st)}
                      onClick={() => updateC({ stages: toggleList(criteria.stages, st) })}
                    />
                  ))}
                </div>
              </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-h-0">
          <div className="px-6 py-3 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-500 mt-2">
              ...
              <span className="font-semibold text-gray-800">{patientPool.length}</span> records
            </p>
          </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {r.patient.age}yo {r.patient.gender} · {r.patient.diagnoses[0]}
                      </p>
                      ...
                      {hasExclusion && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {r.flags
                            .filter((f) => f.toLowerCase().includes("exclusion"))
                            .slice(0, 2)
                            .map((f) => (
                      ...
                      {r.qualifyingDrugs.length > 0 && !hasExclusion && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {r.qualifyingDrugs.map((d) => (
                            <span
                              key={d}
                              className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 bg-violet-100 text-violet-700 rounded-full border border-violet-200 font-medium"
                            >
      ...
      <div className="shrink-0 px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between gap-3 flex-wrap">
        {step !== "criteria" ? (
          <button
            type="button"
          ...
          <button
            ...
            onClick={() => {
              ...
              setBiomarkers(DEFAULT_BIOMARKERS);
            }}
        ...
        {step !== "results" && (
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium transition-colors ml-auto"
          >
            {step === "biomarkers" ? (
              <>
        ...
        {step === "results" && variant === "page" && onPageComplete && (
          <button
            type="button"
            onClick={() => onPageComplete(qualifiedIds)}
            className="flex items-center gap-2 px-5 py-2 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium transition-colors ml-auto"
          >
            Continue to home
        ...
        {step === "results" && !(variant === "page" && onPageComplete) && <div />}
      </div>
    </div>
  );
}
